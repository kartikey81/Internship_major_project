...
def pred():
    ...
    ohlc_df = investpy.get_historical_data(crypto='bitcoin',from_date='01/01/2013',to_date=yesterday_date)
    ...
    final_df = final_df[(final_df['Date'] >= '2013-01-01')]
...
def get_df():
    return investpy.get_crypto_historical_data(crypto='bitcoin',from_date='01/01/2013',to_date=datetime.today().strftime('%d/%m/%Y')).reset_index()[['Date','Close']]
